...
soit $\arg\max(P[X=i])$ avec $P[X=i]$ borné par $[0;1/2]$ pour $i \in \{0,1,2\}$.
...
Il s'agit encore une fois d'une fonction quadratique mais cette fois-ci il y a
...
$$
\ln{(L(\theta))} = \sum_{i=1}^{n}(\ln{(\frac{1}{2\lambda\sqrt{x_i}})}-\frac{\sqrt{x_i}}{\lambda})
$$
